...
import { formatCurrency, generateListingSlug } from "@/lib/utils"; // Added generateListingSlug
...
export const ListingCard = ({ listing, className }: ListingCardProps) => {
  ...
  // Generate slug for the listing URL
  ...
  return (
    <motion.div
      ...
      className="bg-white rounded-xl overflow-hidden shadow-sm border border-gray-200 hover:shadow-md transition-shadow duration-200 group"
    >
      <Link to={`/listings/${listingSlug}`} className="block"> {/* Use slug instead of ID */}
        <Card 
          className={cn(
